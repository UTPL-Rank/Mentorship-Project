...
@Component({
  selector: 'sgm-generate-reports-final',
  templateUrl: './generate-reports-final.component.html',
  styles: [
  ]
})

export class GenerateReportsFinalComponent implements OnInit {
  filterString = "";
  constructor(
  ...
  public allMentors: Observable<Array<SGMMentor.readDTO>> =
    this.route.params.pipe(
      mergeMap((params) =>
        this.mentorsService.getAllMentorsAndShare(params.periodId)
      )
    );

  public areaStats: Observable<Array<AreaStat>> = this.allMentors.pipe(
    map((mentors) => {
